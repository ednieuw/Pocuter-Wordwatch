...
--------------------------------------  
 ...
 Changes V043: Reset preferences if checksum is invalid
 Changes V044:  
Usage of:
...
char BLEbroadcastName[] ="WordClockV043";  // Name of the BLE beacon
#include <Pocuter.h>
...
&nbsp;  L3 = FR, L4 = Wheel<br>
    ...
    <strong>R</strong> Reset settings @ = Reset Pocuter<br>
...
void setup() 
{
 ...
 InitEEPROM();              Tekstprintln("Settings from SD");                                                                       // Load EEPROM settings and check validity 
 ...
 if(strlen(Mem.password)<5 || strlen(Mem.ssid)<3)                                             // If ssid or password invalid turn WIFI/NTP off
    { Mem.WIFIOnOff = Mem.NTPOnOff = 0;  StoreStructInFlashMemory(); }                                // And update EEPROM  }
...
//    StoreStructInFlashMemory();                                                                      // This function uses EEPROM.update() to perform the write, so does not rewrites the value if it didn't change
    }
}
//--------------------------------------------
...
//         sprintf(sptext,"File written!: %s\n",fileName); Tekstprintln(sptext); 
   ...
   } else {  sprintf(sptext,"SDCard not Mounted!\n"); Tekstprintln(sptext);  }

// To be used if SPIFFS works
...
// To be used if Preferences works
...
void GetStructFromFlashMemory(void)
{
  if (pocuter->SDCard->cardIsMounted()) 
  {
     ...
     if (! fp) { sprintf(sptext,"Could not open %s to read!\n", fileName); Tekstprintln(sptext); } 
          ...
          fclose(fp); 
//          sprintf(sptext,"File read!: %s\n",fileName); Tekstprintln(sptext); 
          }    
   } else {  sprintf(sptext,"SDCard not Mounted!\n"); Tekstprintln(sptext);  }

 // sprintf(sptext,"Mem.Checksum = %d\n",Mem.Checksum);Tekstprintln(sptext); 
...
//  USBSerial.printf("Inputstring: %sLengte : %d\n", InputString,InputString.length());
 if(InputString.length()> 30) return;                                                         // If garbage return
...
//                EEPROMstore.write(Mem);                           // Update EEPROM 
 ...
 //            Tekstprintln(sptext); 
    ...
    case 'p':  
             ...
             else sprintf(sptext,"**** Length fault. Enter Prrggbb ****");            
             break;
    case 'q':
    case 'Q':  
             if (InputString.length() == 3 )
...
// sprintf(sptext,"  Display choice: Q%d", Mem.DisplayChoice);                               Tekstprintln(sptext);
...
void StartWIFI_NTP(void)
 ...
 if (WiFi.waitForConnectResult() != WL_CONNECTED) 
      { 
       pocuter->ugui->UG_PutString(0,20,"WiFi Failed!");   
       ...
       Tekstprintln(sptext); 
       pocuter->ugui->UG_PutString(0,20,"WIFI is On"); 
       if(Mem.NTPOnOff)
          {
           NTP.setTimeZone (TZ_Europe_Amsterdam); //\TZ_Etc_GMTp1); // TZ_Etc_UTC
           NTP.begin ();
           Tekstprintln("NTP On"); 
           pocuter->ugui->UG_PutString(0,30,"NTP is On");
          }
