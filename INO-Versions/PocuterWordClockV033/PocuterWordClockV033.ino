...
char BLEbroadcastName[] ="WordClockV033";
#include <Pocuter.h>
...
// Mem.UpperBrightness = _min(Mem.UpperBrightness, 1023); 
 Mem.LanguageChoice  = _min(Mem.LanguageChoice, 4);                                           // Constrain the value to normal ranges 
...
// if(strlen(Mem.password)<5 || strlen(Mem.ssid)<3)                                             // If ssid or password invalid turn WIFI/NTP off
//    { Mem.WIFIOnOff = Mem.NTPOnOff = 0;  EEPROM.put(0,Mem); }                                // And update EEPROM  }
 ...
 if(Mem.WIFIOnOff){ StartWIFI_NTP();  Tekstprintln("WIFI started"); }                           // Start WIFI and NTP if Mem.WIFIOnOff = 1
            ...
            if (InputString.length() == 10 )
              {
                ...
                timeinfo.tm_year = (byte) SConstrainInt(InputString,5,9,2000,9999);
...
void StartBLEService(void)
{
 ...
 TekstSprint("Waiting a client connection to notify...\n"); 
}
//                                                                                            //
//-----------------------------
...
void StartWIFI_NTP(void)
 ...
 if (WiFi.waitForConnectResult() != WL_CONNECTED) { pocuter->ugui->UG_PutString(0,20,"WiFi Failed!");   Tekstprintln("WiFi Failed!");}
 else {
       Tekstprint("Web page started\n");
       sprintf(sptext, "IP Address: %d.%d.%d.%d", WiFi.localIP()[0], WiFi.localIP()[1], WiFi.localIP()[2], WiFi.localIP()[3] );
       Tekstprintln(sptext); 
       pocuter->ugui->UG_PutString(0,20,"WIFI On"); 
       if(Mem.NTPOnOff)
          {
           NTP.setTimeZone (TZ_Europe_Amsterdam); //\TZ_Etc_GMTp1); // TZ_Etc_UTC
           NTP.begin ();
           Tekstprintln("NTP On"); 
           pocuter->ugui->UG_PutString(0,30,"NTP On");
          }
       PrintIPaddressInScreen();
      }
  pocuter->ugui->UG_FontSelect(&FONT_5X8);                                                   // Set back FONT_5X8
 if(Mem.WIFIOnOff) WebPage();
}
