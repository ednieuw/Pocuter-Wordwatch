...
6 EEPROM storage of a struct with settings
7 Light sensor 
...
char BLEbroadcastName[] ="WordClockV034";
#include <Pocuter.h>
...
uint32_t HourColor[24] ={C_WHITE, C_RED,  C_ORANGE, C_YELLOW,  C_YELLOW_GREEN, C_GREEN,
                         C_CYAN,  C_BLUE, C_PURPLE, C_MAGENTA, C_DARK_RED,     C_DARK_GREEN,
                         C_WHITE, C_RED,  C_ORANGE, C_YELLOW,  C_YELLOW_GREEN, C_GREEN,
                         C_CYAN,  C_BLUE, C_PURPLE, C_MAGENTA, C_DARK_RED,     C_DARK_GREEN};  
...
<!DOCTYPE HTML><html><head>
  ...
  .auto-style1 {
   ...
   <span class="auto-style1"><strong>Pocuter Word Clock</strong>

  <p><span class="auto-style1"> Enter Time as: hhmmss (132145)<br>
    <strong>A</strong> Set SSID   <strong>B</strong> Set Password<br>
    <strong>D</strong> Set Date              D15012021)<br>
    <strong>L</strong> L0 = NL, L1 = UK, L2 = DE,<br>
&nbsp;  L3 = FR, L4 = Wheel<br>
    <strong>N</strong> (N2208) Display off between Nhhhh<br>
    <strong>O</strong> Display toggle On/Off<br>
    <strong>P</strong> (P00FF00) own colour design (0-F)<br>
    <strong>Q</strong> Display choice (Q0-6)<br>
    <strong>Q0</strong> = Yellow colour  <strong>Q1</strong> = Colour hourly<br>
    <strong>Q2</strong> = All white colour<strong> Q3</strong> = All Own colour<br>
    <strong>Q4</strong> = Own colour<strong> Q5</strong> = Wheel colour<br>
    <strong>Q6</strong> = Digital display<br>
    <strong>I</strong>  For this info menu<br>
    <strong>R</strong>  Reset settings<br>
    <strong>T</strong> Set Time                (T132145)<br>
    <strong>W</strong>=WIFI, <strong>X</strong>=NTP, <strong>Y</strong>=BLE toggle On/Off <br>
    <br>
...
//1234567890123456789012345678901234567890----  
 char menu[][37] = {
 "A Set SSID B Set Password",
 "D Set Date              (D15012021)",
 "L L0 = NL, L1 = UK, L2 = DE",
 "  L3 = FR, L4 = Wheel",
 "N Display off between Nhhhh (N2208)",
 "O Display toggle On/Off", 
 "P Own colour(0-F)           (P4F8A)",
 "Q Display choice             (Q0-6)",
 " Q0 Yellow    Q1 hourly  colour",
 " Q2 White     Q3 All Own colour",
 " Q4 Own color Q5 = Wheel colour",
 " Q6 Digital display",
 "I To print this Info menu",
 "R Reset settings",
 "T Set Time                (T132145)",
 "W=WIFI, X=NTP, Y=BLE toggle On/Off ", 
...
void setup() 
{
 pocuter->begin(PocuterDisplay::BUFFER_MODE_DOUBLE_BUFFER);                                   // This: PocuterDisplay::BUFFER_MODE_DOUBLE_BUFFER makes the display much faster
 USBSerial.begin(115200);   Tekstprintln("Serial started");                                   // Setup the serial port to 115200 baud //
 InitEEPROM();                                                                              // Load EEPROM settings and check validity 
...
// strcpy(Mem.ssid,"FRITZ!BoxEd");
// strcpy(Mem.password,"eddie2506");
 //strcpy(Mem.ssid,"Guest");                                                                    // Default SSID
 //strcpy(Mem.password,"Guest-001");                                                            // Default password
 strcpy(Mem.ssid,"FRITZ!BoxEd");
 strcpy(Mem.password,"eddie2506");
 StartWIFI_NTP();   Tekstprintln("WIFI started");                                             // Start WIFI and NTP if Mem.WIFIOnOff = 1
...
//--------------------------------------------
// ARDUINO Check connected input devices
//--------------------------------------------
...
void EverySecondCheck(void)
{
 ...
 uint32_t msLeapButton = millis()- LastButtonTime;
 static int lumi=0;
 (msLeap>500?lumi=(1000-msLeap):lumi=msLeap);                                                 // Turn OFF the second on LED. 
 if (msLeap%40==0) pocuter->setStatusLED(25,lumi/40,0);                                       // Do not update the LED unnecesary often
...
void SerialCheck(void)
{
 ...
 while (USBSerial.available())
    { 
     char c = USBSerial.read();                                   //     Serial.write(c);
...
void InitEEPROM(void)
{
 ...
 EEPROM.begin(sizeof(EEPROMstorage));
 return; 
 EEPROM.get(0,Mem);                                                                            // Get the data from EEPROMstorage
// if( Mem.Checksum != 25065) Reset();                                                          // If the checksum is NOK the EEPROM was not filled
...
// Mem.UpperBrightness = _min(Mem.UpperBrightness, 1023); 
 strcpy(Mem.ssid,"FRITZ!BoxEd");
 strcpy(Mem.password,"eddie2506");
 Mem.LanguageChoice  = 4;      //_min(Mem.LanguageChoice, 4);                                           // Constrain the value to normal ranges 
 Mem.DisplayChoice   = _min(Mem.DisplayChoice, DIGITAL);                                      // Constrain the value to normal ranges 
 if(Mem.OwnColour == 0) Mem.OwnColour = 0X002345DD;                                           // If memory is empty cq black colour then store default value, blue  
 EEPROM.put(0,Mem);  
 
 // Override settings for testing 
 Mem.DisplayChoice = DEFAULTCOLOUR;                                                           // Any strange value in EEPROM? Set to default display
 Mem.OwnColour = C_YELLOW;                                                                    // If memory is empty then store default value  
// Mem.LanguageChoice = 4;
 Mem.TimeCorrection = 0;
...
void Reset(void)
{
 ...
 Mem.OwnColour       = C_YELLOW;                                                              // Own designed colour. Default Blue  
...
//--------------------------------------------
void Tekstprint(char const tekst[])   { USBSerial.print(tekst);       SendMessageBLE(tekst);} // { printf(tekst); SendMessageBLE(tekst); }
void Tekstprintln(char const tekst[]) { sprintf(sptext,"%s\n",tekst); Tekstprint(sptext);   }
void TekstSprint(char const tekst[])  { USBSerial.print(tekst);   printf(tekst);            } //Serial.printf("endTransmission: %u\n", error);  
...
{
 USBSerial.printf("Inputstring: %sLengte : %d\n", InputString,InputString.length());
 if(InputString.length()> 20) { Tekstprintln("String lenth too long >19"); return;}
 if(InputString[0] > 32 && InputString[0] <127)                                                // Does the string start with a valid character?
   ...
   switch (InputString[0])
   {
    ...
    case 'a': 
            if (InputString.length() >5 )
            {
             ...
             sprintf(sptext,"SSID set to: %s", Mem.ssid);  
            }
            else sprintf(sptext,"**** Length fault. Use more than 4 characters ****");
            break;
    case 'B':
    case 'b': 
           if (InputString.length() >5 )
            {  
             ...
             sprintf(sptext,"Password is: %s", Mem.password);  
            }
            else sprintf(sptext,"**** Length fault. Use more than 4 characters ****");
            break;   
    ...
    case 'd':  
            if (InputString.length() == 10 )
              {
               timeinfo.tm_mday   = (byte) SConstrainInt(InputString,1,3,0,23);
               timeinfo.tm_mon    = (byte) SConstrainInt(InputString,3,5,0,59); 
               timeinfo.tm_year   = (byte) SConstrainInt(InputString,5,9,2000,9999)-1900;
               SetRTCTime();
               Print_tijd();  //Tekstprintln(sptext);                                           // Print_Tijd() fills sptext with time string
              }
            else sprintf(sptext,"**** Length fault. Enter ddmmyyyy ****");
            break;
    case 'I':
    case 'i': 
             SWversion();
...
//                EEPROMstore.write(Mem);                           // Update EEPROM 
             ...
             else sprintf(sptext,"**** Display choice length fault. Enter L0 - L3"); 
            break;     
 ...
 //            Tekstprintln(sptext); 
    ...
    case 'p':  
             ...
             else sprintf(sptext,"**** Length fault. Enter Pwwrrggbb ****");            
             break;
    case 'q':
    case 'Q':  
             if (InputString.length() == 3 )
...
//                                                                                            //
             if(InputString.length() == 8)  // T125500
               {
                timeinfo.tm_hour   = (byte) SConstrainInt(InputString,1,3,0,23);
                timeinfo.tm_min = (byte) SConstrainInt(InputString,3,5,0,59); 
                timeinfo.tm_sec = (byte) SConstrainInt(InputString,5,7,0,59);
                SetRTCTime();
                Print_tijd(); 
               }
    ...
    case '2':        
             if (InputString.length() == 7 )                                                  // For compatibility input with only the time digits
              {
               timeinfo.tm_hour   = (byte) SConstrainInt(InputString,0,2,0,23);
               timeinfo.tm_min = (byte) SConstrainInt(InputString,2,4,0,59); 
               timeinfo.tm_sec = (byte) SConstrainInt(InputString,4,6,0,59);
 ...
 switch (Mem.DisplayChoice)
  {
   ...
   case OWNHETISCLR  : LetterColor = Mem.OwnColour; 
                       ...
                       SECColor = LetterColor;                                      break;    // Own colour except HET IS WAS  
...
{ 
 ...
 sprintf(sptext," %s %s %s", Mem.WIFIOnOff?"WIFI=On":"WIFI-Off", 
                             Mem.NTPOnOff? "NTP=On":"NTP-Off",
                             Mem.BLEOnOff? "BLE=On":"BLE-Off" );                          Tekstprintln(sptext);
...
{ 
 if (PrintDigital) 
  ...
  {
   byte Language;
   uint32_t DimmedLetterColour = 0X191919;  //C_MIDNIGHT_BLUE;                                                                          // Start by clearing the display to a known state 
   SetSecondColour();
   ...
   switch(Language)   // Print all the character in the backgound color, a sort of ClearScreen
     {
      ...
      strncpy(Template,"HETVISOWASOVIJFQPRECIESZSTIENKPFKWARTSVOORSOVERAHALFSMIDDERTVIJFATWEESOEENOXVIERELFQTIENKTWAALFBHDRIECNEGENACHTFZESVZEVENOENVUUR",129);  
              ColorLeds(Template,0,127, DimmedLetterColour);
...
//                                                                                            //
//--------------------------------------------
// POCUTER Button   Change the time with  Buttons
//--------------------------------------------
...
void StartBLEService(void)
{
 ...
 //Tekstprintln("Waiting a client connection to notify..."); 
}
//                                                                                            //
//-----------------------------
// POCUTER BLE  CheckBLE
//------------------------------
void CheckBLE(void)
{
...
// if (millis() > LastButtonTime + 150) 
  { 
//   if (Button1) ButtonPressed+=1;
//   if (Button2) ButtonPressed+=2;
//   if (Button3) ButtonPressed+=4;   
...
//  pocuter->ugui->UG_FillFrame(0, 0, sizeX - 1, sizeY - 1, C_DARK_BLUE);
 ...
 pocuter->ugui->UG_PutString(0,10,"Starting") ;
}
...
{ 
 pocuter->ugui->UG_FillScreen(C_BLACK);
...
void ShowChars(uint32_t BackgroundColor)
{
 ...
 for(int y=0;y<MATRIX_HEIGHT;y++)
    for(int x=0;x<MATRIX_WIDTH;x++)
         pocuter->ugui->UG_PutChar ( Strippos[LEDnr].Character , 6*x, 7*y , Strippos[LEDnr++].RGBColor , BackgroundColor ) ;
}
//                                                                                            //
//--------------------------------------------
//  POCUTER SCREEN 
...
void ShowChars(void)
{
 ...
 for(int y = 0; y < MATRIX_HEIGHT; y++)
    for(int x = 0; x < MATRIX_WIDTH; x++, LEDnr++)
         pocuter->ugui->UG_PutChar(Strippos[LEDnr].Character, 6*x, 8*y, Strippos[LEDnr].RGBColor, C_BLACK );
}
//--------------------------------------------
...
{
// if( timeinfo.tm_sec==0)      ClearScreen();
 GetTijd(0); 
 ClearScreen(); 
 if(Mem.WIFIOnOff) PrintIPaddressInScreen();                                                   // If WIFI is on print the IP-address
 sprintf(sptext,"%02d:%02d:%02d",timeinfo.tm_hour,timeinfo.tm_min,timeinfo.tm_sec);
 ...
 pocuter->ugui->UG_PutString(10,22,sptext);
 ...
 pocuter->ugui->UG_SetBackcolor(C_BLACK);
 pocuter->ugui->UG_PutString(17,55,sptext);
}
//--------------------------------------------
// POCUTER  SCREEN 
...
void StartWIFI_NTP(void)
 {
 if(Mem.WIFIOnOff==0) return;
  pocuter->ugui->UG_PutString(0,20,"Connecting");
 if( (strlen(Mem.ssid) <5) || (strlen(Mem.password)<7) ) return;

 pocuter->ugui->UG_PutString(0,30,"+/-30 sec");
 WiFi.mode(WIFI_STA);
 WiFi.begin (Mem.ssid, Mem.password);
 if (WiFi.waitForConnectResult() != WL_CONNECTED) { pocuter->ugui->UG_PutString(0,30,"WiFi Failed!");   Tekstprintln("WiFi Failed!");}
 else {
       Tekstprint("Web page started  ");
       sprintf(sptext, "IP Address: %d.%d.%d.%d", WiFi.localIP()[0], WiFi.localIP()[1], WiFi.localIP()[2], WiFi.localIP()[3] );
       Tekstprintln(sptext); 
       WebPage();
       pocuter->ugui->UG_PutString(0,40,"WIFI On"); 
       Tekstprintln("WIFI On"); 
       if(Mem.NTPOnOff)
          {
           NTP.setTimeZone (TZ_Europe_Amsterdam); //\TZ_Etc_GMTp1); // TZ_Etc_UTC
           NTP.begin ();
           Tekstprintln("NTP On"); 
           pocuter->ugui->UG_PutString(0,50,"NTP On");
          }
