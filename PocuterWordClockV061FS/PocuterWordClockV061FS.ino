...
--------------------------------------  
 ...
 Changes V054: Added case '&': ,-->Get NTP time, in menu. 
               Corrected tm_mon + 1. Was wrong one month
 Changes V055: Added toggle StatusLED On/Off
 Changes V056: Turn off StatusLEDs pocuter->setStatusLED(0,0,0); when switching status in menu (option P)
 Changes V057: Added analogue clock
 Changes V058: Added WiFi.onEvent(WiFiEvent); and  WiFiEvent() when WIFI is lost
 Changes V059: Added OTA
 Changes V060: if(WiFi.localIP()[0]==0) tries to reconnect every hour
 Changes V061: Solved If Analogue clock turned on again the out ring was not drawn. Added function RestartDisplay();
...
Patition Scheme: Minimal SPIFFS (1.9Mb with OTA)
...
uint8_t ScreenMiddleX = 48;
uint8_t ScreenMiddleY = 32;
/*-----------------------------
...
void setup() 
{
 ...
 Mem.UseSDcard = 1;                                                                          // SD-card is used for storages of settings
                                          Tekstprintln("Pocuter fast display started");      // pocuter->begin(PocuterDisplay::BUFFER_MODE_DOUBLE_BUFFER);                                   // PocuterDisplay::BUFFER_MODE_DOUBLE_BUFFER makes the display much faster
                     #else 
...
void EverySecondCheck(void)
{
 ...
 if (msLeap%100==0  && Mem.StatusLEDOnOff) 
     pocuter->setStatusLED(15-lumi,lumi*WIFIConnected,0);                                     // Do not update the LED unnecesary often
...
//                                   printf("Minute %ld\n",millis());
 ...
 Print_tijd(); Tekstprintln(sptext);                                                                                             //  sprintf(sptext,"NTP:%s", NTP.getTimeDateString());   Tekstprintln(sptext);  
...
void Reset(void)
{
 ...
 Mem.UseBLELongString= 0;                                                                     // Fast BLE off
 Mem.NTPOnOff        = 0;                                                                     // NTP On
 Mem.WIFIOnOff       = 0;                                                                     // WIFI On  
 Mem.StatusLEDOnOff  = 1;                                                                     // Fast BLE Off
 ...
 if (pocuter->SDCard->cardIsMounted()) 
   {
     ...
     if (! fp) { sprintf(sptext,"Could not open %s to read\n", fileName); Tekstprintln(sptext); } 
           ...
           fclose(fp); 
           sprintf(sptext,"Settings from SD-card read: %s",fileName); Tekstprintln(sptext); 
          }    
   } 
                     #ifndef FASTSCREEN 
  else 
...
//    sprintf(sptext,"SDCard not Mounted!\n Using Flashstorage"); Tekstprintln(sptext);  
    ...
    FLASHSTOR.end();         
                    ...
                    #endif   // FASTSCREEN
  sprintf(sptext,"Mem.Checksum = %d and is %s",Mem.Checksum,Mem.Checksum==25065?"OK":"Not OK" );     Tekstprintln(sptext); 
 ...
 switch (Mem.DisplayChoice)
  {
   case DEFAULTCOLOUR: LetterColor = C_YELLOW;
                       MINColor    = C_YELLOW;
                       SECColor    = C_YELLOW;                                          break;// Yellow                                                                       
...
//--------------------------------------------                                                //
// POCUTER  SCREEN Restart display after been turned off
//--------------------------------------------
...
void PrintTimeInScreen(void) 
{
 GetTijd(0); 
// ClearScreen(); 
// if(Mem.WIFIOnOff && !IP_Printed) 
    { PrintIPaddressInScreen(); IP_Printed = true; }                                                  // If WIFI is on print the IP-address
 sprintf(sptext,"%02d:%02d:%02d",timeinfo.tm_hour,timeinfo.tm_min,timeinfo.tm_sec);
...
// tft.fillScreen((C_GRAY)); 
// pocuter->ugui->UG_SetForecolor((C_WHITE), (C_GRAY));  // Adding a background colour erases previous text automatically
ClearScreen();
 pocuter->ugui->UG_SetForecolor(C_LIME);
 pocuter->ugui->UG_SetBackcolor(C_BLACK);
 pocuter->ugui->UG_FillCircle(smx, smy, 30, (C_GREEN));                  // Draw clock face
 pocuter->ugui->UG_FillCircle(smx, smy, 28, (C_BLACK));
 for(int i = 0; i<360; i+= 30)                                            // Draw 12 lines
   {
    ...
    x11 = sx*25+smx;
    y11 = sy*25+smy;
    pocuter->ugui->UG_DrawLine(x00, y00, x11, y11, (C_GREEN));
   }
 for(int i = 0; i<360; i+= 6)                                                   // Draw 60 dots
   {
    ...
    x00 = sx*28+smx;
    y00 = sy*28+smy;
    pocuter->ugui->UG_DrawPixel(x00, y00, (C_WHITE));                                 // Draw minute markers
    if(i==0  || i==180) pocuter->ugui->UG_FillCircle(x00, y00, 2, (C_WHITE));     // Draw main quadrant dots
    if(i==90 || i==270) pocuter->ugui->UG_FillCircle(x00, y00, 2, (C_WHITE));
   }
//  tft.fillCircle(160, 121, 3, (C_WHITE));
...
// static uint16_t osx=smx, osy=smy, omx=smx, omy=smy, ohx=smx, ohy=smy;  // Saved H, M, S x & y coords
 ...
 pocuter->ugui->UG_FontSelect(&FONT_4X6); 
 sprintf(sptext,"%02d:%02d:%02d",timeinfo.tm_hour,timeinfo.tm_min,timeinfo.tm_sec);
 pocuter->ugui->UG_FillFrame(28, 20, 70, 26 , C_BLACK);
 pocuter->ugui->UG_SetForecolor(C_YELLOW);
 pocuter->ugui->UG_SetBackcolor(C_BLACK);
 pocuter->ugui->UG_PutString(28,20,sptext);
 
// tft.setCursor(smx-50, smy/2);
// Displayprint(sptext); 
 DrawSecondHand(ss,     smx, smy, C_RED);   // (C_RED));
 DrawMinuteHand(mm, 0,  smx, smy, C_WHITE);    // (C_WHITE)
 DrawHourHand(  hh, mm, smx, smy, C_WHITE_SMOKE); 
 DrawSecondHand(ss,     smx, smy, C_RED);
}
...
{  
 if(ss <0 ) ss = 59;                                    
 uint16_t osx = cos((ss * 6 - 90) * 0.0174532925) * 22 + smx;    // 0-59 ss -> 0-354 degrees
 uint16_t osy = sin((ss * 6 - 90) * 0.0174532925) * 22 + smy; 
 pocuter->ugui->UG_DrawLine(osx,   osy,   smx, smy, Kleur);    
 pocuter->ugui->UG_FillCircle(smx, smy, 2, C_RED);
}
...
void DrawMinuteHand(int mm, int ss, uint8_t smx, uint8_t smy, uint32_t Kleur)
{
 ...
 uint16_t omx = cos((mdeg-90)*0.0174532925) * 22 + smx;    
 uint16_t omy = sin((mdeg-90)*0.0174532925) * 22 + smy;
// sprintf(sptext,"MINUUT: mm:%d ss:%d ohx:%d ohy:%d ",mm,ss,omx,omy ); Tekstprintln(sptext);
 ...
 pocuter->ugui->UG_DrawLine(omx - xcorr, omy - ycorr, smx - xcorr, smy - ycorr, Kleur);  
 pocuter->ugui->UG_DrawLine(omx,         omy,         smx,         smy,         Kleur); 
 pocuter->ugui->UG_DrawLine(omx + xcorr, omy + ycorr, smx + xcorr, smy + ycorr, Kleur);
...
void DrawHourHand(uint8_t hh, int mm, uint8_t smx, uint8_t smy, uint32_t Kleur)
{
 ...
 uint16_t  ohx = cos((hdeg-90)*0.0174532925) * 16 + smx;    
 uint16_t  ohy = sin((hdeg-90)*0.0174532925) * 16 + smy;
// sprintf(sptext,"   UUR: hh:%d mm:%d ohx:%d ohy:%d ",hh,mm,ohx,ohy ); Tekstprintln(sptext);
 ...
 pocuter->ugui->UG_DrawLine(ohx-xcorr, ohy-ycorr, smx-ycorr, smy-ycorr, Kleur); 
 pocuter->ugui->UG_DrawLine(ohx,       ohy,       smx, smy, Kleur);
 pocuter->ugui->UG_DrawLine(ohx+xcorr, ohy+ycorr, smx+xcorr, smy+xcorr, Kleur);     
}
